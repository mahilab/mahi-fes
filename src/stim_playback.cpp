...
volatile sig_atomic_t interrupt = false; // gloabl interrupt variable
...
int server(void)
{
	...
	FILE *flog;   // create a pointer to the file to read open loop control data in from    	
		flog = fopen("/home/michael/Desktop/pulse_log.txt", "r");	// open the file for reading data
	   	if (flog == NULL)
	   	{
	       	printf("Error opening the pulse width data file\n");
	       	return 0;
	    }	

	i = 0;
	// Copy data from text file to float array
	while(fscanf(flog, "%d", &pulsewidth) > 0)
	{
	...
	FILE *fp;   // create a pointer to the file for data logging
    	fp = fopen("/home/michael/Desktop/playback_data.txt", "w");	// open the file for data logging
    	if (fp == NULL)
    	{
	...
	if(elapsed_t > 5) // stop stimulation once trajectory time has elapsed
	{
		close(sockfd);		
		fclose(fp);
		//fclose(flog);
